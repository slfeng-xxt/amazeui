import React from 'react'
import PropTypes from 'prop-types'
import { connect } from 'dva'
import { Helmet } from 'react-helmet' // 可复用head
import { Footer } from '../../components/WXLayout'

function Income({ location, dispatch, income, loading }) {
  let {
    curpath,
  } = income
  return (
    <div>
      <Helmet>
        <title>牛视科技公众号 -- niusee.cn</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=0, minimum-scale=1.0, maximum-scale=1.0" />
        <meta name="renderer" content="webkit" />
        <meta charset="utf-8" />
        <link rel="stylesheet" href="/css/user/user.css" />
      </Helmet>
      <div className="content">
        <div className="content-prompt clearfix">
          购买内容总收益（元）<span className="pull-right">6.00</span>
        </div>
        <ul className="contert-main">
          <p className="contert-main-title">2017-5-19</p>
          <li>
            <div className="media">
                <div className="media-left">
                  <a href="#">
                    <img className="media-object" data-src="holder.js/64x64" alt="64x64" src="data:image/svg+xml;base64," data-holder-rendered="true" />
                  </a>
                </div>
                <div className="media-body">
                  <h5 className="media-heading">与学生深度沟通技</h5>
                  <p className="clearfix"><span className="pull-left">100人购买</span></p>
                </div>
                <div className="media-right">
                  +2.00
                </div>
              </div>
          </li>
        </ul><ul className="contert-main">
          <p className="contert-main-title">2017-5-19</p>
          <li>
            <div className="media">
                <div className="media-left">
                  <a href="#">
                    <img className="media-object" data-src="holder.js/64x64" alt="64x64" src="data:image/svg+xml;base64," data-holder-rendered="true" />
                  </a>
                </div>
                <div className="media-body">
                  <h5 className="media-heading">与学生深度沟通技</h5>
                  <p className="clearfix"><span className="pull-left">100人购买</span></p>
                </div>
                <div className="media-right">
                  +200
                </div>
              </div>
          </li>
        </ul>
      </div>
      <Footer curpath={curpath} />
    </div>
  )
}

export default connect(
  ({ income, loading}) => ({ income, loading: loading.models.income })
)(Income)
